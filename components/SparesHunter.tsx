...
import { ArrowLeftIcon, CheckCircleIcon, UploadIcon, CameraIcon, TrashIcon, MailIcon, PhoneIcon } from './common/Icon';
...
const SparesHunter: React.FC<SparesHunterProps> = ({ onNavigate }) => {
  ...
  const saveSparesRequestToSupabase = async () => {
    ...
    const requestToSave = {
      ...
      car_model: data.carModel !== (currentModels.length > 0 ? currentModels[0] : '') ? data.carModel : null,
      // part_photo_base64: data.partPhotoBase64, // Consider Supabase Storage
  ...
  const stepTitles: Record<SparesStep, string> = {
      ...
      case SparesStep.DETAILS:
        return (
          <form className="space-y-5">
            <div>
              <label htmlFor="partDescription" className="block text-sm font-medium text-slate-300">{translate('shDescribePart')}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300">{translate('shUploadPartPhoto')}</label>
            ...
            <Button onClick={handleNextStep} fullWidth>{translate('next')}</Button>
          </form>
        );
      case SparesStep.CONTACT_INFO:
        return (
          <form className="space-y-5">
            ...
            <Button onClick={handleNextStep} isLoading={isLoading} fullWidth>{translate('shFindMyPart')}</Button>
          </form>
        );
      case SparesStep.CONFIRMATION:
        ...
        return (
          <div className="text-center space-y-4">
            ...
            <p className="text-slate-300">{translate('shSearchConfirmation', "...", { contactMethod: contactMethod || 'your provided contact' })}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-8 min-h-[calc(100vh-10rem)]">
      ...
      <div className="bg-slate-800 shadow-2xl rounded-xl p-6 sm:p-8">
        <h2 className="text-2xl font-bold text-center text-white mb-2">{translate('sparesHunterTitle')}</h2>
        <p className="text-sm text-center text-slate-400 mb-1">{translate('sparesHunterDesc')}</p>
        <p className="text-xs text-center text-emerald-400 font-semibold mb-6">{translate('freeService')}</p>
        
        <p className="text-md font-semibold text-center text-sky-300 mb-6">{stepTitles[step]}</p>
