...
import { WrenchIcon, UsersIcon, CheckCircleIcon, CarIcon, BatteryIcon, WrenchScrewdriverIcon, MailIcon, PhoneIcon, UserIcon } from './common/Icon';
      ...
      <section className="text-center py-16 px-4 bg-gradient-to-b from-slate-900 to-slate-800/70">
        <div className="max-w-4xl mx-auto">
          ...
          <Button 
            size="xl" 
            variant="primary" 
            onClick={() => { setIsDemoModalOpen(true); setIsDemoSubmitted(false); setFormError(null); }}
      ...
      >
        {isDemoSubmitted ? (
          ...
          <form onSubmit={handleDemoSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="user_name" className="block text-sm font-medium text-slate-300">{translate('yourName')}</label>
            ...
            <div>
              <label htmlFor="garage_name" className="block text-sm font-medium text-slate-300">{translate('garageName')}</label>
               <div className="relative mt-1">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"> <WrenchIcon className="h-4 w-4 text-slate-400" /> </div>
                <input type="text" name="garage_name" id="garage_name" value={demoForm.garage_name || ''} onChange={handleInputChange} required
                        className="w-full bg-slate-700 border-slate-600 rounded-md shadow-sm py-2 pl-9 pr-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm text-white" />
              </div>
            </div>
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-slate-300">{translate('emailAddress')}</label>
              <div className="relative mt-1">
            ...
            <div>
              <label htmlFor="phone" className="block text-sm font-medium text-slate-300">{translate('phoneNumber')} ({translate('optional', 'Optional')})</label>
              <div className="relative mt-1">
            ...
            <div className="pt-2 flex justify-end gap-3">
                <Button type="button" variant="ghost" onClick={() => setIsDemoModalOpen(false)} disabled={isDemoSubmitting}>
                    {translate('close')}
                </Button>
                <Button type="submit" variant="primary" isLoading={isDemoSubmitting} disabled={isDemoSubmitting}>
                    {translate('submit')}
                </Button>
            </div>
          </form>
