...
import { ArrowLeftIcon, CheckCircleIcon, MailIcon, PhoneIcon } from './common/Icon';
  ...
  const handleNextStep = async () => {
    switch (step) {
      case PredictionStep.DETAILS:
        if (validateDetails()) {
          setIsLoading(true);
          setTimeout(() => { // Simulate getting preliminary insight
      ...
      case PredictionStep.DETAILS:
        return (
          <form className="space-y-5">
            <div>
              <label htmlFor="carModel_text" className="block text-sm font-medium text-slate-300">{translate('vdSelectCarModel')}</label>
              <select id="carModel_text" value={data.carModel_text || ''} onChange={(e) => handleChange('carModel_text', e.target.value)}
                className="mt-1 block w-full bg-slate-700 border-slate-600 rounded-lg shadow-sm py-3 px-4 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-white">
                <option value="" disabled>{translate('vdSelectCarModel')}</option>
                {CAR_BRANDS_MODELS.flatMap(brand => brand.models.filter(model => !model.toLowerCase().includes("select ") && !model.toLowerCase().includes("first select"))).slice(0,20)
            ...
            <div>
              <label htmlFor="mileage" className="block text-sm font-medium text-slate-300">{translate('bpMileage')}</label>
            ...
            <div>
              <label htmlFor="avgDrivingDistance" className="block text-sm font-medium text-slate-300">{translate('bpAvgDriving')}</label>
            ...
            <div>
              <label htmlFor="chargingMethod" className="block text-sm font-medium text-slate-300">{translate('bpChargingMethod')}</label>
            ...
            <Button onClick={handleNextStep} isLoading={isLoading} fullWidth>{translate('bpGetPreliminaryInsight')}</Button>
          </form>
        );
      case PredictionStep.PRELIMINARY_INSIGHT:
        return (
          <div className="text-center space-y-5">
            ...
            <p className="text-slate-300 bg-slate-700/50 p-4 rounded-md">{data.preliminaryInsight}</p>
            <Button onClick={handleNextStep} fullWidth variant="primary">{translate('next')}</Button>
          </div>
        );
      case PredictionStep.CONTACT_INFO:
        return (
          <form className="space-y-5">
            <h3 className="text-lg font-semibold text-white text-center">{translate('bpContactForReport')}</h3>
             <div>
              <label htmlFor="contactEmail" className="block text-sm font-medium text-slate-300">{translate('contactEmail')}</label>
             ...
             <div>
              <label htmlFor="contactPhone" className="block text-sm font-medium text-slate-300">{translate('contactPhone')}</label>
              <div className="relative mt-1">
            ...
            <Button onClick={handleNextStep} isLoading={isLoading} fullWidth>{translate('submit')}</Button>
          </form>
        );
      case PredictionStep.CONFIRMATION:
        ...
        return (
          <div className="text-center space-y-4">
            ...
            <h3 className="text-xl font-semibold text-white">{translate('thankYou')}</h3>
            <p className="text-slate-300">{translate('bpReportConfirmation', "...", { contactMethod: contactMethod || 'your provided contact' })}</p>
            {data.id && <p className="text-xs text-slate-500 mt-1">Request ID: {data.id}</p>}
            <Button onClick={() => onNavigate(AppView.LANDING)} fullWidth variant="ghost">{translate('backToHome')}</Button>
          </div>
        );
    }
  };
  
  const stepTitles: Record<PredictionStep, string> = {
    [PredictionStep.DETAILS]: translate('bpEnterDetails'),
    [PredictionStep.PRELIMINARY_INSIGHT]: translate('bpPreliminaryInsightTitle'),
    [PredictionStep.CONTACT_INFO]: translate('enterContactDetails'),
    [PredictionStep.CONFIRMATION]: translate('confirmation'),
  };

  return (
    <div className="max-w-2xl mx-auto p-4 md:p-8 min-h-[calc(100vh-10rem)]">
      ...
      <div className="bg-slate-800 shadow-2xl rounded-xl p-6 sm:p-8">
        <h2 className="text-2xl font-bold text-center text-white mb-2">{translate('batteryPredictionTitle')}</h2>
        <p className="text-sm text-center text-slate-400 mb-6">{stepTitles[step]}</p>

        {error && <p className="text-red-400 bg-red-900/30 p-3 rounded-md text-sm mb-4 text-center">{error}</p>}
        {renderStepContent()}
      </div>
    </div>
  );
